# 🚀 Flavio Leotta - Portfolio

[![GitHub Pages](https://img.shields.io/badge/GitHub%20Pages-Live-brightgreen)](https://yourusername.github.io/THEANY1703)
[![HTML5](https://img.shields.io/badge/HTML5-E34F26?logo=html5&logoColor=white)](https://developer.mozilla.org/en-US/docs/Web/HTML)
[![CSS3](https://img.shields.io/badge/CSS3-1572B6?logo=css3&logoColor=white)](https://developer.mozilla.org/en-US/docs/Web/CSS)
[![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?logo=javascript&logoColor=black)](https://developer.mozilla.org/en-US/docs/Web/JavaScript)

## 👨‍💻 Chi Sono

Ciao! Sono **Flavio Leotta**, un **IT Specialist & DevOps Engineer** di 22 anni con una passione sfrenata per l'innovazione tecnologica. Ho completato la mia esperienza come Full Stack Developer a febbraio 2025 e da allora ricopro il ruolo di IT Specialist & DevOps Engineer, oltre a essere uno sviluppatore a tutto tondo, con un approccio pratico e una visione sistemistica completa.

### 💡 Competenze Acquisite e Consolidate

- **Progettazione di infrastrutture di rete aziendali** — dalla pianificazione topologica alla configurazione di apparati e sicurezza
- **Progettazione e gestione di architetture server** — implementazione di ambienti virtualizzati con Proxmox e VMware, ottimizzati per stabilità, scalabilità e performance
- **Esperienza trasversale** nello sviluppo, nel deployment e nella manutenzione di sistemi complessi

## 🛠️ Tecnologie e Competenze

### Frontend Development
- **Expert**: Vue.js, TypeScript, JavaScript ES6+, HTML5, CSS3
- **Advanced**: SASS/SCSS, Tailwind CSS, Bootstrap
- **Intermediate**: Webpack, Vite

### Backend Development
- **Expert**: Node.js, Express.js, Java, Python
- **Advanced**: Django, C++, C#
- **Intermediate**: Laravel, Spring Boot

### Mobile Development
- **Expert**: React Native, Flutter
- **Intermediate**: Ionic, Cordova

### Database & Storage
- **Expert**: PostgreSQL, MySQL
- **Advanced**: MongoDB, Redis
- **Intermediate**: Elasticsearch, SQLite

### DevOps & Cloud
- **Expert**: Docker, GitLab CI/CD, Linux
- **Advanced**: Kubernetes, AWS, Azure, Proxmox
- **Intermediate**: Terraform, Ansible

### IT Infrastructure
- **Expert**: Data Center Design, Network Security, Server Management
- **Advanced**: Cisco Networking, Firewall Configuration, VPN Setup
- **Intermediate**: Load Balancing

### Tools & Software
- **Expert**: Git, VS Code, Windows, Linux, Debian, Kali, Active Directory
- **Advanced**: IntelliJ IDEA, Postman, Office 365
- **Intermediate**: Figma, Adobe Creative Suite

## 🚀 Esperienza Professionale

### IT Specialist & DevOps Engineer
**Febbraio 2025 - Presente**
- Progettazione e implementazione di infrastrutture di rete aziendali
- Gestione di architetture server virtualizzate con Proxmox e VMware
- Automazione e orchestrazione di sistemi complessi
- Deployment e manutenzione di ambienti di produzione

### Full Stack Developer Freelancer
**Agosto 2022 - Febbraio 2025**
- Sviluppo di applicazioni web full-stack con Vue.js, Laravel, Node.js, Express.js, Flutter,  
- Implementazione di architetture microservizi (Vari pacchetti api e anche di pagamento es. Stripe, MyPos, Paypal)
- Gestione di database complessi e ottimizzazione delle performance (Oracle DB, MYSql, SQLite, MongoDB)
- Deployment automatizzato con CI/CD pipelines (Azure, GitLab, GCC)

### IT Security Specialist
**Giugno 2021 - Settembre 2021** - *Security Service S.R.L.*
- Installazione e configurazione di sistemi di sicurezza avanzati
- Progettazione di reti sicure per aziende
- Manutenzione di infrastrutture IT critiche

## 🎓 Formazione & Certificazioni

- **Tecnico Perito Informatico** - ITS Archimede, Catania (2017 - 2022)
- **Cisco NetAcad** - Cybersecurity Fundamentals & Linux System Administration (2020 - 2021)
- **Certificazioni Cloud** - AWS & Azure, Cloud Architecture & DevOps Practices (2023 - Presente)
- **Certificazioni HTB** - Corsi su pentesting e difesa di infrastrutture informatiche enterprise (2019 - Presente)
- **Corsi Udemy** - Vari corsi Udamy sia sul lato sviluppo che sul lato reti (2019 - Presente)


## 📞 Contatti

- **Email**: [giorgifla1703@gmail.com](mailto:giorgifla1703@gmail.com)
- **LinkedIn**: [Flavio Leotta](https://www.linkedin.com/in/flavio-leotta-96372b112/)
- **Instagram**: [@leottaflavio](https://www.instagram.com/leottaflavio/)
- **GitLab**: [@1L_c0nt38](https://gitlab.com/1L_c0nt38)
- **Telefono**: (+39) 3491004744

## 🧠 Visione

Con una visione moderna e una forte passione per tecnologia, automazione e innovazione, porto un approccio sistemistico completo a ogni progetto.

## 📄 Licenza

Questo progetto è open source e disponibile sotto la [MIT License](LICENSE).

---

**Made with ❤️ and lots of ☕ by Flavio Leotta**

*Ultimo aggiornamento: Settembre 2025*
